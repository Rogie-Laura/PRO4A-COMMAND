...
export function mapPersonnelRow(row: Record<string, string>): PersonnelRecord {
  return {
    rank: row.Rank ?? "",
...
export function toCountItems(counts: Map<string, number>, total: number): CountItem[] {
...
export function buildOfficeBreakdown(records: PersonnelRecord[]): OfficeBreakdownItem[] {
...
export function buildKpis(records: PersonnelRecord[]): KpiMetric[] {
  return [
    {
      id: "total",
...
export function buildWorkforceSummary(records: PersonnelRecord[]): WorkforceSummary {
...
export function buildRankDistribution(records: PersonnelRecord[]): RankDistribution {
  return {
...
export function buildAgeDistributionByOffice(
  records: PersonnelRecord[],
): OfficeAgeDistributionRow[] {
  return OFFICES.map((office) => {
...
export function buildUnitRows(records: PersonnelRecord[]): UnitRow[] {
  const total = records.length
...
export function buildLeadership(records: PersonnelRecord[]): LeadershipGroups {
  return {
...
export function buildStatusStats(records: PersonnelRecord[]): CountItem[] {
  return toCountItems(countBy(records, (r) => r.pStatus), records.length).slice(0, 6)
}

export function buildGenderStats(records: PersonnelRecord[]): CountItem[] {
