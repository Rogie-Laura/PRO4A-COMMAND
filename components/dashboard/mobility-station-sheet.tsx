...
type MobilityStationSheetProps = {
  office: VehicleOfficeBreakdownItem | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

const chartConfig = {
  count: { label: "Vehicles", color: "var(--chart-1)" },
}
...
function estimateLabelWidth(stations: VehicleStationBreakdownItem[], compact: boolean) {
...
function StationTooltip({
  ...
  active?: boolean
  payload?: Array<{ payload: VehicleStationBreakdownItem }>
}) {
    ...
    <div className="grid min-w-40 gap-1.5 rounded-lg border border-border/50 bg-background px-3 py-2 text-xs shadow-xl">
      ...
      <div className="grid gap-1 text-muted-foreground">
        ...
        <div className="flex justify-between gap-4">
          ...
          <span className="font-mono font-medium text-foreground tabular-nums">
            {item.operational.toLocaleString()}
          </span>
        </div>
        <div className="flex justify-between gap-4">
          <span>Non-operational</span>
          <span className="font-mono font-medium text-foreground tabular-nums">
            {item.nonOperational.toLocaleString()}
          </span>
        </div>
      </div>
    </div>
...
export function MobilityStationSheet({
  office,
  open,
  onOpenChange,
}: MobilityStationSheetProps) {
  ...
  const totals = useMemo(() => {
    if (!office) return { count: 0, operational: 0, nonOperational: 0 }
    return office.stations.reduce(
      (acc, item) => ({
        count: acc.count + item.count,
        operational: acc.operational + item.operational,
        nonOperational: acc.nonOperational + item.nonOperational,
      }),
      { count: 0, operational: 0, nonOperational: 0 },
    )
  }, [office])
            ...
            <SheetHeader className="border-b pb-4">
              <div className="flex items-center gap-3 pr-8">
                ...
                <div className="min-w-0">
                  ...
                  <SheetDescription>
                    {office.stations.length} sub-units · {totals.count.toLocaleString()} vehicles
                    · {totals.operational.toLocaleString()} operational
                  </SheetDescription>
                </div>
              </div>
            </SheetHeader>
            ...
            <div className="pb-8 pl-0 pr-4">
              {office.stations.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">
                  Walang vehicle data para sa office na ito.
                </p>
              ) : chartReady ? (
